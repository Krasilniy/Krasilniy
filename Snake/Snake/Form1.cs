    ...
    public partial class Form1 : Form
    {
        ...
        private void _eatItself()
        {
            for (int _i = 1; _i < _score; _i++)
            {
                ...
                {
                    for (int _j = _i; _j <= _score; _j++)
                        this.Controls.Remove(snake[_j]);
                    _score = _score - (_score - _i + 1);
                    labelScore.Text = "Score: " + _score;
                }
            }
        }
        ...
        private void _eatFruit()
        {
            ...
            {
                labelScore.Text = "Score: " + ++_score;
                snake[_score] = new PictureBox();
                snake[_score].Location = new Point(snake[_score - 1].Location.X + 40 * dirX, snake[_score - 1].Location.Y - 40 * dirY);
                snake[_score].Size = new Size(_sizeOfSides - 1, _sizeOfSides - 1);
                snake[_score].BackColor = Color.Red;
